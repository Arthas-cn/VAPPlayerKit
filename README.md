...
如果宿主需要按 tag 指定字体，可在 `DynamicContentProvider` 中实现 `font(forTag:)` 并返回 `UIFont`；返回 `nil` 时继续使用自动字号估算。自动估算最多缩小三次，仍放不下时默认由 UIKit 的 `byTruncatingTail` 截断。将 `PlaybackOptions.dynamicTextOverflowMode` 设为 `.marquee` 后改为从右向左跑马灯；`marqueeSpeed`（默认 80 pt/s）和 `marqueeStartDelay`（默认 0.6s）可调，每个滚动周期开头都会停顿，并从文字真正上屏开始计时。循环间隙固定 25 pt。文字能放下时始终静态居中。跑马灯时钟跟随 pause / suspend，与视频 `loopCount` 循环无关。超长文字超出纹理预算时回退截断。
...
Objective-C provider 也可实现可选的 `fontForTag:`，为文字 tag 指定 `UIFont`；未实现或返回 `nil` 时使用同样的自动估算。文字溢出默认截断，可通过 `VPKPlaybackOptions.dynamicTextOverflowMode` 切换为跑马灯。
